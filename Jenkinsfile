pipeline {
    ...
    parameters {
        choice(choices: ['MySql','MSSQL', 'MSSQLDC', 'Postgres', 'Oracle','winjump'], description: 'Select the Solution to build', name: 'solution')
  ...
  def build_solution(sol) {
	...
	dir(path) {
	    ...
	    if (params.Build) {
            ...
            println total_count
            sh script: "$tf_cmd apply  -var-file=$path"  + "/main.tfvars" + " -var vsphere_password=" + '${VC_PASS}'	 + " -var ansible_key=" + '${SSH_KEY}'	+	 " -var infoblox_pass=" + '${INFOBLOX_PASS}'  +	" -var vm_count=" + total_count
