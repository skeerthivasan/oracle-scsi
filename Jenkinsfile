pipeline {
    ...
    parameters {
        choice(choices: ['MySql','MSSQL', 'Postgres', 'Oracle','winjump'], description: 'Select the Solution to build', name: 'solution')
  ...
  def build_solution(sol) {
	...
	dir(path) {
        ...
        if (params.Install) {
            ...
            println "------------------"
            // if sol.trim() == 'MSSQL':
                sh script: "ansible-playbook -i hosts.ini ../../ansible/playbooks/" + sol.trim().toLowerCase() + "-install.yml -vvv"
            // else:
