pipeline {
    ...
    parameters {
        choice(choices: ['MySql','MSSQL', 'MSSQLDC', 'Postgres', 'Oracle','winjump','logrhythm','syslog','qradar','demo'], description: 'Select the Solution to build', name: 'solution')
	...
	dir(path) {
        ...
        if (params.Install) {
            ...
            if (solname == 'MSSQLDC') {
                ...
                sh script: "ansible-playbook -i hosts.ini ../../ansible/playbooks/" + solname.toLowerCase() + "-install.yml" +  "-e ansible_python_interpreter=/usr/bin/python3"
            } 
            if  (solname == 'Oracle') {
            }
            else {
